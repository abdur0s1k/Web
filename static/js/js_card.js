  ...
  function updateLikedItemsList() {
    ...
    if (Object.keys(likedProducts).length === 0) {
      console.log("Нет лайкнутых товаров.");
      return;
    }
    ...
    for (const [productId, productName] of Object.entries(likedProducts)) {
      ...
      removeButton.addEventListener("click", async () => {
        try {
          ...
          // Удаляем товар из локального объекта
          delete likedProducts[productId];
          likeCount--;
          document.getElementById("likeCount").textContent = likeCount;
          ...
          const likeButton = document.querySelector(
            `.products__card-btn[data-product-id="${productId}"]`
          );
          if (likeButton) {
            const heartIcon = likeButton.querySelector(".heart-icon");
            if (heartIcon) {
              heartIcon.classList.remove("liked"); // Убираем выделение иконки
              console.log(`Состояние лайка сброшено для товара: ${productId}`);
            } else {
              console.warn(`Иконка лайка не найдена для товара: ${productId}`);
            }
          } else {
            console.warn(`Кнопка лайка не найдена для товара: ${productId}`);
          }
          ...
          console.log(`Товар успешно удалён: ${productId}`);
        } catch (error) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    // Приведение данных из localStorage к единому виду (замена id на product_id)
    const checkoutItems = JSON.parse(localStorage.getItem("checkoutBasketItems")) || [];
    checkoutItems.forEach(item => {
        if (item.id && !item.product_id) {
            item.product_id = item.id; // Преобразование устаревшего ключа
            delete item.id;
        }
    });
    localStorage.setItem("checkoutBasketItems", JSON.stringify(checkoutItems));

    const checkoutTotal = localStorage.getItem("checkoutBasketTotal") || "0.00";

    const itemsList = document.getElementById("checkoutItemsList");
    const totalPrice = document.getElementById("checkoutTotalPrice");

    if (itemsList && totalPrice) {
        checkoutItems.forEach(item => {
            if (!item.product_id || !item.name || !item.price || !item.quantity) {
                console.error("Некорректные данные товара:", item);
                return;
            }
            item.price = (item.price * item.quantity).toFixed(2);
            const listItem = document.createElement("li");
            listItem.textContent = `${item.name} - $${item.price} (${item.quantity} шт)`;
            itemsList.appendChild(listItem);
        });

        totalPrice.textContent = `$${checkoutTotal}`;
    }

    const view = localStorage.getItem("checkoutView");
    const checkoutSummary = document.getElementById("checkoutSummary");
    const productInfo = document.getElementById("productInfo");

    if (checkoutSummary && productInfo) {
        ...
        if (view === "summary") {
            ...
            console.error("Неизвестное значение view:", view);
        }

        localStorage.removeItem("checkoutView");
    }

    const deliveryOption = document.getElementById("delivery-option");
    const cardForm = document.getElementById("card-form");

    if (deliveryOption && cardForm) {
        deliveryOption.addEventListener("change", function () {
            cardForm.style.display = deliveryOption.value === "delivery" ? "block" : "none";
        });
    }
    ...
    if (checkoutForm) {
        checkoutForm.addEventListener("submit", async function (event) {
            ...
            if (viewMode === "product") {
                const productNameElement = document.querySelector("#productInfo .checkout-info span");
                const productPriceElement = document.querySelector("#productInfo .checkout-info:nth-of-type(2) span");
                const productIdElement = document.getElementById("product-id");

                if (!productIdElement) {
                    alert("Ошибка: отсутствует ID товара.");
                    return;
                }

                const productName = productNameElement ? productNameElement.textContent.trim() : null;
                let productPriceRaw = productPriceElement ? productPriceElement.textContent.trim() : null;
                const productId = productIdElement.value.trim();

                if (productPriceRaw) {
                    productPriceRaw = productPriceRaw.replace(/[^\d.,-]/g, '').replace(',', '.').trim();
                }
                const productPrice = parseFloat(productPriceRaw);

                if (!productName || isNaN(productPrice) || !productId) {
                    console.error("Ошибка в данных товара:", { productName, productPrice, productId });
                    alert("Ошибка в данных товара. Пожалуйста, проверьте шаблон.");
                    return;
                }
                ...
                orderData.products = checkoutItems.map(item => ({
                ...
                alert("Неизвестный режим оформления заказа.");
                return;
            }
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    localStorage.removeItem("checkoutBasketItems");
                    localStorage.removeItem("checkoutBasketTotal");
    ...
    function validateCardForm() {
        ...
        const expiryMonth = document.getElementById("card-expiry-mm").value.trim();
        const expiryYear = document.getElementById("card-expiry-yy").value.trim();
        ...
        if (!expiryMonth || expiryMonth.length !== 2 || isNaN(expiryMonth) || +expiryMonth < 1 || +expiryMonth > 12) {
            alert("Введите корректный месяц (MM).");
            return false;
        }

        if (!expiryYear || expiryYear.length !== 2 || isNaN(expiryYear) || +expiryYear < currentYear) {
            alert("Введите корректный год (YY).");
            return false;
        }
